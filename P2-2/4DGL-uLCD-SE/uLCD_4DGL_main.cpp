...
void uLCD_4DGL :: writeBYTE(const char c)   // send a BYTE command to screen
{
    ...
    _cmd.write(&c, 1);
    wait_us(1000);  //mbed is too fast for LCD at high baud rates in some long commands
...
void uLCD_4DGL :: writeBYTEfast(const char c)   // send a BYTE command to screen
{
    ...
    _cmd.write(&c, 1);
    wait_us(1000);  //mbed is too fast for LCD at high baud rates - but not in short commands
